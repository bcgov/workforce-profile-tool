...
class Main extends Component<Props> {
  getOccupationData(data: FixTypeLater): FixTypeLater {
    return data
      ? data.filter((r: FixTypeLater) => r['Variable_Type'] !== 'Region')
      : data
  }

  getRegionData(data: FixTypeLater): FixTypeLater {
    return data
      ? data.filter((r: FixTypeLater) => r['Variable_Type'] !== 'Management')
      : data
  }
  ...
  render(): JSX.Element {
    ...
    let activeInnerTab
    if (
      activeOuterTab === 'representation' &&
      !this.props.match.params.lowLevelNav
    ) {
      activeInnerTab = 'by-occupation'
    } else {
      activeInnerTab =
        this.props.match.params.lowLevelNav || 'representation-by-group'
    }
          ...
          >
            <Tab key={'home'} name="Home">
              <div>
                ...
                <p className="lead">
                  ...
                  the paper report that was previously produced which can be
                  found on the{' '}
                  <a
                ...
                <h2>Caveats:</h2>
                <p className="lead">
                  This tool uses data from the BC Public Service Work
                  Environment Survey (WES), which was conducted in February,
                  2018. Therefore, the data on this site may not match other
                  reports produced by the Public Service Agency regarding the
            ...
            <Tab key={'indicators-of-progress'} name="Indicators of Progress">
              <div className="Secondary">
                ...
                >
                  <Tab
                    key={'representation-by-group'}
                    name="By Designated Group"
                  >
                    <Progress
                  ...
                  <Tab key={'by-occupation'} name="By Occupation">
                    <Occupation
                      ...
                      data={this.getOccupationData(
                        this.props.data.occupationRegionData
                      )}
                      employeeCount={employeeCount}
                    />
                  </Tab>
