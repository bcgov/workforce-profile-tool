...
class MinistryGraph extends Component<Props> {
  render(): JSX.Element {
...
class MinistrySubGraph extends Component<SubgraphProps> {
  render(): JSX.Element {
    if (!this.props.data) return <div>&nbsp;</div>
    ...
    const provincialRepresentation = parseFloat(
      this.props.data[0]['BC Population']
    )
    ...
    const chartData = categories.sort().map((category) => {
      const count = this.props.data.map(
        (row) => +parseFloatClean(row[category])
      )[0]

      if (count === 0) hasSuppressedData = true

      color = COLOR_MAP[this.props.data[0] as string]['Des_Grp']
